...
(function() {
	...
	app.Record = Backbone.Model.extend({
		...
		url: function() {
			return '/admin/mediafiles/'+ (this.isNew() ? '' : this.id +'/');
		}
	});
	...
	app.HeaderView = Backbone.View.extend({
		...
		addNew: function() {
			if (this.$el.find('[name="name"]').val() === '') {
			...
			else if (this.$el.find('[name="filename"]').val() === '') {
				...
				this.model.save({
					...
					lifecycle: 1,
					action: 1,
					filename: this.$el.find('[name="filename"]').val(),
					lastCheckedBy: 'n/a',
					geoLocation: app.geoLocation.latitude + " " + app.geoLocation.longitude,
					actionLog: []
				},{
					success: function(model, response) {
						if (response.success) {
							app.headerView.model.set({name: '' , lifecycle: '', action:'', filename: '', lastCheckedBy: '', geoLocation:'', actionLog: [] });
	...
	app.ResultsView = Backbone.View.extend({
		...
		render: function() {
			...
			this.collection.each(function(record, index) {
	...
	app.ResultsRowView = Backbone.View.extend({
		...
		events: {
			...
			'click .btn-inspect': 'inspect'
		},
		viewDetails: function() {
			location.href = this.model.url();
		},
		inspect: function() {
			location.href = this.model.url() + 'actions/';
		},
		render: function() {
			this.$el.html(this.template( this.model.attributes ));
			return this;
